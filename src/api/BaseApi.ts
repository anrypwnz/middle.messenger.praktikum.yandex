...
class BaseApi {
    ...
    get(endpoint: `/${string}`, options?: Record<string, unknown>): Promise<Response> {
        return this._http.get(this.getPath() + endpoint, this.handleOptions(options))
            .then(this.handleResponse)
    }

    post(endpoint: `/${string}`, options?: Record<string, unknown>): Promise<Response> {
        return this._http.post(this.getPath() + endpoint, this.handleOptions(options))
            .then(this.handleResponse)
    }

    put(endpoint: `/${string}`, options?: Record<string, unknown>): Promise<Response> {
        return this._http.put(this.getPath() + endpoint, this.handleOptions(options))
            .then(this.handleResponse)
    }

    delete(endpoint: `/${string}`, options?: Record<string, unknown>): Promise<Response> {
